"""Tests de lib.processor — persistencia de processed/{identifier}/ y catalog_index.yaml.
...
def test_write_processed_primera_llamada_valida_crea_index_y_articulos_y_actualiza_catalog(
    ...
    index_path = tmp_path / "processed" / IDENTIFIER / "index.md"
    article_path = (
        tmp_path / "processed" / IDENTIFIER / "articles" / f"{IDENTIFIER}-01.md"
    )
...
def test_write_processed_dos_llamadas_articulos_distintos_index_con_union(
    ...
    index = processor.read_index(IDENTIFIER, tmp_path)
    ...
    assert (
        tmp_path / "processed" / IDENTIFIER / "articles" / f"{IDENTIFIER}-02.md"
    ).exists()
